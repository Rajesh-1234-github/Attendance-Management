...
def manually_fill():
    ...
    def fill_attendance():
        ...
        global subb
        subb = SUB_ENTRY.get()
        DB_table_name = str(subb + "" + Date + "_Time" +
                            Hour + "" + Minute + "" + Second)
    ...
    if l1 == '':
        ...
        try:
            ...
            detector = cv2.CascadeClassifier(
               'C:\\Users\\hp\Downloads\\Final Project\\face\\face\\haarcascade_frontalface_default.xml')

               
            #detector = cv2.CascadeClassifier('C:/haarcascade_frontalface_default.xml')
            #if detector.empty(): 
              #  raise IOError("Error loading cascade classifier xml file.")
            Enrollment = txt.get()
            Name = txt2.get()
            sampleNum = 0
            while (True):
            ...
            with open('C:\\Users\\hp\\Downloads\\Final Project\\face\\face\\StudentDetails\\StudentDetails.csv','a+') as csvFile:
               # 
             #  ('./StudentDetails/StudentDetails.csv', 'a+')
    ...
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                df = pd.read_csv("C:\\Users\\hp\\Downloads\\Final Project\\face\\face\\StudentDetails\\StudentDetails.csv")
                cam = cv2.VideoCapture(0)
                ...
                date_for_DB = datetime.datetime.fromtimestamp(
                    ts).strftime('%Y_%m_%d')
                DB_Table_name = str(
                    Subject + "" + date_for_DB + "_Time" + Hour + "" + Minute + "" + Second)
                import pymysql.connections
